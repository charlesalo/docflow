...
**Candidate:** Charles Marvin San Juan Alo (charlesmsjalo@gmail.com)
...
## Note on Assignment Structure (this attempt)

This is a single project deliverable, not multiple independent tasks. All five numbered capability areas (document editing, file upload, sharing, persistence, product/engineering quality) plus the AI workflow note and walkthrough video are required parts of one submission. Scope decisions and tradeoffs are logged in `ARCHITECTURE.md`; AI usage is logged in `AI_WORKFLOW.md` (summary) and `notes/ai-usage-log.md` (raw). Countdown shown in the assessment portal (~239 minutes / ~4 hours) is treated as the real time constraint, taking precedence over the brief's stated "4-6 hours."